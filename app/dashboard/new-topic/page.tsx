...
function NewTopic() {
  ...
  const regenerateMainOutline = useCallback(async () => {
    if (!topic) return;
    setLoading(true);
    setMainOutlineHTML("");
    setMainOutlineStreamingDone(false);
    // Clear cache
    delete mainOutlineCache.current[topic];
    try {
      await handleBreakdown(topic);
    } catch (err: any) {
      setError(err.message || "Failed to regenerate main outline");
      setLoading(false);
    }
  }, [topic, handleBreakdown]);

  const regenerateSubOutline = useCallback(async () => {
    if (!selectedSubtopic || !topic) return;
    setLoading(true);
    setSubOutlineHTML("");
    setSubOutlineStreamingDone(false);
    // Clear cache
    delete subOutlineCache.current[selectedSubtopic];
    try {
      await handleSelectSubtopic(selectedSubtopic);
    } catch (err: any) {
      setError(err.message || "Failed to regenerate sub-outline");
      setLoading(false);
    }
  }, [selectedSubtopic, topic, handleSelectSubtopic]);

  const regenerateNotes = useCallback(async () => {
    if (!selectedSubSubtopic || !selectedSubtopic) return;
    setLoading(true);
    setFinalContent("");
    // Clear cache
    delete finalContentCache.current[selectedSubSubtopic];
    try {
      await handleSelectSubSubtopic(selectedSubSubtopic);
    } catch (err: any) {
      setError(err.message || "Failed to regenerate notes");
      setLoading(false);
    }
  }, [selectedSubSubtopic, selectedSubtopic, handleSelectSubSubtopic]);
  ...
  const regenerateDiveDeeper = useCallback(async () => {
    if (!followUpQuestion) return;
    setDiveDeeper("");
    setIsDivingDeeper(true);
    try {
      await handleDiveDeeper();
    } catch (err: any) {
      toast.error("Failed to regenerate dive deeper");
      setIsDivingDeeper(false);
    }
  }, [followUpQuestion, handleDiveDeeper]);
            ...
            {!loading && finalContent && (
              <>
                <button
                  ...
                  className="flex items-center justify-center px-3 py-2 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 rounded-md text-sm font-medium transition-colors w-full sm:w-auto"
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <button
                ...
                disabled={isQuizzing}
                className="flex-1 sm:flex-none inline-flex items-center justify-center px-3 py-2 border border-indigo-200 dark:border-indigo-700 shadow-sm text-sm font-medium rounded-md text-indigo-700 dark:text-indigo-300 bg-white dark:bg-gray-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <RotateCcw className={`w-4 h-4 mr-1 ${isQuizzing ? 'animate-spin' : ''}`} />
                {isQuizzing ? 'Generating...' : 'Regenerate'}
              </button>
              <button
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              >
                ...
                {isDivingDeeper ? 'Generating...' : 'Regenerate'}
              </button>
              <button
                onClick={() => setNotesView("notes")}
                className="flex-1 sm:flex-none inline-flex items-center justify-center px-3 py-2 border border-indigo-200 dark:border-indigo-700 shadow-sm text-sm font-medium rounded-md text-indigo-700 dark:text-indigo-300 bg-white dark:bg-gray-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors whitespace-nowrap"
              ...
              {breadcrumb.onClick ? (
                <button
                  ...
                  className={`text-sm font-medium px-3 py-2 rounded-md transition-colors flex items-center ${
                    breadcrumb.current
                      ? 'text-white bg-indigo-600 font-bold shadow-sm border border-transparent'
  ...
  const renderMainOutline = () => (
    ...
    >
      ...
      {!loading && mainOutlineStreamingDone && (
        <>
          <div className="mb-6 p-4 bg-indigo-600 border-l-4 border-indigo-800 rounded-r-lg flex items-start shadow-sm">
